...
public:
	...
	void setNode(int, NodeWeight, int, bool, bool);
	...
	void setShortPath(int L);
	int getShortPath() const;
	void setPredecessor2(int p);
	int getPredecessor2() const;

private:
	int id, shortPath, predecessor2;
	NodeWeight weight;
	bool marked;
	bool visited;
};
...
// Copy constructor
{
	setNode(n.getId(), n.getWeight(), n.getShortPath(), n.isMarked(), n.isVisited());
}
...
void node::setNode(int id, NodeWeight w = 1, int SP = 1000000, bool m = false, bool v = false)
// Initialize a node;
{
	setId(id);
	setWeight(w);
...
void node::setShortPath(int L) {
	shortPath = L;
}

int node::getShortPath() const{
	return shortPath;
}

void node::setPredecessor2(int p) {
	predecessor2 = p;
}

int node::getPredecessor2() const {
	return predecessor2;
}
...
ostream &operator<<(ostream &ostr, const node &n)
{
	ostr << "node: " << n.getId() << " weight: " << n.getWeight()
		<< " visited: " << n.isVisited() << " marked: " << n.isMarked() << " predecessor2: " << n.getPredecessor2() << " shortest path: " << n.getShortPath() << endl;
...
void edge::setEdge(int i, int j, EdgeWeight w = 1)
...
public:
	...
	int addNode(NodeWeight w = 1);
	int addNode(node n);

	void setEdgeWeight(int i, int j, EdgeWeight w = 1);
	EdgeWeight getEdgeWeight(int i, int j) const;

	NodeWeight getTotalNodeWeight();
	EdgeWeight getTotalEdgeWeight();

	void setNodeWeight(int i, NodeWeight w = 1);
	...
	bool hasUnvisitedNeighbors(int n);

	void findPathRecursive(int n);
	bool findShortestPath1();
	int RecPath(int n);
	int RecPathSize();
	int NonrecPath();
	int NonrecPathSize();
	bool isNonrecPathEmpty();
	int Predecessor(int n);

	bool findShortestPath2();
	int Predecessor2(int n);
	void setShortPath(int n, int S);
...
private:
	...
	int NumEdges;
	bool done;
	vector<int> recurPath;
	queue<int> nonrecurPath;
	vector<int> predecessor;

	struct Lessthan {
		bool operator()(node* x, node* y) const {
			if (((*y).getShortPath() > 100000) && ((*x).getShortPath() > 100000)) {
				return (*y).getId() < (*x).getId();
			}
			else {
				return ((*y).getShortPath() < (*x).getShortPath());
			}
		}
	};
...
void graph::findPathRecursive(int n) {
	int N;
	visit(n);
	if (n == numNodes() - 1) {
		recurPath.push_back(n);
		done = true;
	}
	if (done == true) {
...
bool graph::findShortestPath1() {
	...
	while ((!nonrecurPath.empty())) {
		v = nonrecurPath.front();

		while (hasUnvisitedNeighbors(v)) {
			for (int i = 0; i < numNodes(); i++) {
				if (isEdge(v, i)) {
					w = edges[v][i].getDest();
...
bool graph::findShortestPath2() {
	int v, w, temp, temp2, SP, start = 0, end = (numNodes() - 1);

	clearVisit();

	priority_queue<node*, vector<node*>, Lessthan> PATH;
	...
	setShortPath(start, 0);
	nodes[start].setPredecessor2(0);
			...
			while (hasUnvisitedNeighbors(v)) {
				for (int i = 0; i < numNodes(); i++) {
					if (isEdge(v, i)) {
						w = edges[v][i].getDest();
						visit(w);
						temp = nodes[v].getShortPath() + 1;
						temp2 = nodes[w].getShortPath();
						SP = min(temp2, temp);
						setShortPath(w, SP);
						if (temp < temp2) {
							nodes[w].setPredecessor2(v);
						}
					}
				}
			}
...
void graph::setShortPath(int n, int S) {
	nodes[n].setShortPath(S);
}

bool graph::hasUnvisitedNeighbors(int n) {
	int w;
...
int graph::RecPath(int n) {
	return recurPath[n];
}

int graph::RecPathSize() {
	return recurPath.size();
}

int graph::NonrecPath() {
	int N = nonrecurPath.front();
	nonrecurPath.pop();
	return N;
}

int graph::NonrecPathSize() {
	return nonrecurPath.size();
}

bool graph::isNonrecPathEmpty() {
	return nonrecurPath.empty();
}

int graph::Predecessor(int n) {
	return predecessor[n];
}

int graph::Predecessor2(int n) {
	return nodes[n].getPredecessor2();
}
